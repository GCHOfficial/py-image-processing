...
class PyGUI(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.statusbar.setFont(QtGui.QFont("Sans Serif", 8))
        ...
        if self.ui.pushButton.text() == "Clear Selection":
            ...
            if self.ui.radioButton.isChecked():
                ...
                self.ui.statusbar.showMessage(
                    "JPG to PNG selected. Select input files location"
                )
                ...
                self.filenames.clear()
                self.ui.statusbar.showMessage(
                    "PNG to JPG selected. Select input files location"
                )
                dlg = QtWidgets.QFileDialog()
                dlg.setFileMode(QtWidgets.QFileDialog.ExistingFiles)
                ...
                self.filenames.clear()
                self.ui.statusbar.showMessage(
                    "GIF creation selected. Select input files location"
                )
                dlg = QtWidgets.QFileDialog()
                dlg.setFileMode(QtWidgets.QFileDialog.ExistingFiles)
                ...
                self.ui.statusbar.showMessage(
                    "Please select an action using the radio buttons"
                )
    ...
    def handle_convert_button(self):
        if (
            ...
            if self.filenames == []:
                self.ui.statusbar.showMessage("Load input files first.")
            else:
    ...
    def success_message_box(self):
        ...
        msg.setIcon(1)
        msg.setText("Image(s) converted succesfully!")
    ...
    def image_conversion(self):
        try:
            ...
            msg = QtWidgets.QMessageBox()
            msg.setIcon(3)
            msg.setText("The following image could not be converted:")
            msg.setInformativeText(message)
            msg.setWindowTitle("Unidentified image error")
            msg.exec_()
            self.ui.statusbar.showMessage(
                "Unidentified image in input. Try using a different one!"
            )
